...
public interface UpdatableSortedMap<K, V> extends Collection<Entry<K, V>>, UpdatableMap<K, V>, SortedMap<K, V> {
    /**
     * Returns a new updatable sorted map containing all the entries present in the first map, but not in any of the
     * other provided maps.
     *
     ...
     * @return A new updatable sorted map containing all the entries present in the first map, but not in any of the
     *         other provided maps.
     */
    static <L, W> UpdatableSortedMap<L, W> differenceOf(final Comparator<? super L> comparator,
    ...
    /**
     * Returns a new updatable sorted map containing all the entries present in each of the provided maps.
     *
     ...
     * @return A new updatable sorted map containing all the entries present in each of the provided maps.
     */
    static <L, W> UpdatableSortedMap<L, W> intersectionOf(final Comparator<? super L> comparator,
            final Map<? extends L, ? extends W>... maps) {
        if (maps.length == 0) {
            return empty(comparator);
        }
    ...
    /**
     * Returns a new updatable sorted map cloned from the provided map with the specified key and value cardinality.
     *
     * @param <L>                    The key type.
     * @param <W>                    The value type.
     * @param keyAndValueCardinality The key and value cardinality.
     * @param comparator             The comparator by which to sort the keys.
     * @param map                    The original map.
     * @return A new updatable sorted map cloned from the provided map with the specified key and value cardinality.
     */
    static <L, W> UpdatableSortedMap<L, W> of(final KeyAndValueCardinality keyAndValueCardinality,
            final Comparator<? super L> comparator, final Map<? extends L, ? extends W> map) {
        return new UpdatableSortedTreeMap<L, W>(keyAndValueCardinality, comparator, map);
    }
