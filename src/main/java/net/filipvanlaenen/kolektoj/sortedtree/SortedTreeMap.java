...
/**
 * An implementation of the {@link net.filipvanlaenen.kolektoj.SortedMap}
 * interface backed by a sorted tree, in particular an AVL tree.
 *
...
public final class SortedTreeMap<K, V> implements SortedMap<K, V> {
	...
	/**
	 * The comparator to use for comparing entries using the keys only in this
	 * sorted map.
	 */
	...
	/**
	 * Constructor taking the key and value cardinality and the entries as its
	 * parameter.
	 *
	...
	public SortedTreeMap(final KeyAndValueCardinality keyAndValueCardinality, final Comparator<K> comparator,
		...
		if (keyAndValueCardinality == DISTINCT_KEYS) {
			this.entries = ArrayUtilities.quicksort(ArrayUtilities.cloneDistinctElements(entries),
	...
	private Object[] compact(final Object[] originalEntries) {
		...
		ElementCardinality cardinality = keyAndValueCardinality == DUPLICATE_KEYS_WITH_DUPLICATE_VALUES
				? DUPLICATE_ELEMENTS
				: DISTINCT_ELEMENTS;
