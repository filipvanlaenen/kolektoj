...
public interface ModifiableSortedMap<K, V>
    ...
    /**
     * Returns a new modifiable sorted map containing all the entries present in the first map, but not in any of the
     * other provided maps.
     *
     ...
     * @return A new modifiable sorted map containing all the entries present in the first map, but not in any of the
     *         other provided maps.
     */
    static <L, W> ModifiableSortedMap<L, W> differenceOf(final Comparator<? super L> comparator,
     ...
     */
    static <L, W> ModifiableSortedMap<L, W> empty(final Comparator<? super L> comparator) {
        return new ModifiableSortedTreeMap<L, W>(comparator);
    }

    /**
     * Returns a new modifiable sorted map containing all the entries present in each of the provided maps.
     *
     ...
     * @return A new modifiable sorted map containing all the entries present in each of the provided maps.
     */
    static <L, W> ModifiableSortedMap<L, W> intersectionOf(final Comparator<? super L> comparator,
            final Map<? extends L, ? extends W>... maps) {
        if (maps.length == 0) {
            return empty(comparator);
        }
     ...
     */
    static <L, W> ModifiableSortedMap<L, W> of(final Comparator<? super L> comparator, final Entry<L, W>... entries) {
     ...
     */
    static <L, W> ModifiableSortedMap<L, W> of(final Comparator<? super L> comparator, final L key, final W value) {
     ...
     */
    static <L, W> ModifiableSortedMap<L, W> of(final Comparator<? super L> comparator, final L key1, final W value1,
    ...
    /**
     * Returns a new modifiable sorted map with the specified comparator containing all the entries from the provided
     * maps.
     *
     ...
     * @return A new modifiable sorted map with the specified key and value cardinality containing all the entries from
     *         the provided maps.
     */
    static <L, W> ModifiableSortedMap<L, W> unionOf(final Comparator<? super L> comparator,
            final Map<? extends L, ? extends W>... maps) {
    ...
    /**
     * Returns a new modifiable sorted map containing all the entries from the provided maps, sorted the same way as the
     * first map.
     *
     * This method corresponds to the union operation in set theory, denoted by the symbol ∪, with {1, 2, 3} ∪ {2, 3, 4}
     * = {1, 2, 3, 4}. For multisets, allowing duplicate elements, {1, 2, 3} ∪ {2, 3, 4} = {1, 2, 2, 3, 3, 4}.
     ...
     * @param maps      The maps from which to copy all the entries.
     * @return A new modifiable sorted map containing all the entries from the provided maps, sorted the same way as the
     *         first map.
     */
    static <L, W> ModifiableSortedMap<L, W> unionOf(final SortedMap<L, ? extends W> sortedMap,
            final Map<? extends L, ? extends W>... maps) {
